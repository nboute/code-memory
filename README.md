...
- **Keepalive (important):** WSL2 shuts the VM down about a minute after the last session detaches — even with systemd services running — which stops dockerd and the containers. Whenever docker is reached through WSL (freshly provisioned or pre-existing), the installer offers a logon keepalive holding a hidden persistent session (`wsl.exe -e sleep infinity`), keeping the VM — and dockerd — alive; `restart: unless-stopped` brings the `cm-*` containers back whenever dockerd (re)starts. It installs as a user scheduled task `code-memory-wsl-docker` (registered through the Task Scheduler COM API, which unlike `schtasks /SC ONLOGON` normally works unelevated, and fires seconds after logon), falling back to a Startup-folder `code-memory-wsl-docker.vbs` when the Task Scheduler is locked down — Startup entries work everywhere but ride the Startup-apps stagger, which can be minutes into a heavy corporate logon. Disable it with `Unregister-ScheduledTask -TaskName code-memory-wsl-docker` (or by deleting the file from `shell:startup`). Without it, any `wsl` command revives everything in a few seconds, but the MCP server/hooks would see connection-refused in between.
